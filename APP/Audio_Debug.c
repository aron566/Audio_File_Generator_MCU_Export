...
static int16_t Audio_Temp_Data[8*AUDIO_DEBUG_FRAME_MONO_SIZE];
/*发送设置*/
...
void Audio_Debug_Put_Data(const int16_t *Left_Audio_Data, const int16_t *Right_Audio_Data, uint8_t Channel_Number, ...)
{
  ...
  for(uint32_t i = 0; i < AUDIO_DEBUG_FRAME_MONO_SIZE; i++)
  {
    ...
    {
      case CHANNEL_1_EN:
        Audio_Temp_Data[index++] = Left_Audio_Data[i];
        Audio_Temp_Data[index++] = Left_Audio_Data[i];
        break;
      case CHANNEL_2_EN:
        Audio_Temp_Data[index++] = Left_Audio_Data[i];
        Audio_Temp_Data[index++] = Right_Audio_Data[i];
        break;
      case CHANNEL_3_EN:
      case CHANNEL_4_EN:
      case CHANNEL_5_EN:
      ...
      case CHANNEL_8_EN:
      {
        ...
        {
          Audio_Temp_Data[index++] = (va_arg(args, uint16_t *))[i];
        }
        break; 
      }
      default:
        Audio_Temp_Data[index++] = 0;
        Audio_Temp_Data[index++] = 0;
        break;
    }
  }
  va_end(args);
  CQ_16putData(&CQ_Audio_Data_Handle, (const uint16_t *)Audio_Temp_Data, (2 + Channel_Number) * AUDIO_DEBUG_FRAME_MONO_SIZE);
}
